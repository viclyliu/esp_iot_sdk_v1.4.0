...
user_esp_platform_upgrade_begin(struct espconn *pespconn, struct upgrade_server_info *server)
{
    ...
    ESP_DBG("viclyinfo::test...");
    ESP_DBG("viclyinfo::pre_version£º%s==>upgrade_version£º%s",server->pre_version,server->upgrade_version);
...
user_esp_platform_init(void)
{
	...
	//----------------------------------------------------------------s
	os_printf("flash:esp_param.devkey = %s\n",esp_param.devkey);
	os_printf("flash:esp_param.token = %s\n",esp_param.token);


	char mykey[40]="f8a1e8f214cbf7d697925d8ad1941ccd4cfeb302";
	 os_memcpy(esp_param.devkey,mykey, 40);
	 os_memcpy(esp_param.token,mykey, 40);

	os_printf("vicly:esp_param.devkey = %s\n",esp_param.devkey);
	os_printf("vicly:esp_param.token = %s\n",esp_param.token);

	//f8a1e8f214cbf7d697925d8ad1941ccd4cfeb302
	//-------------------------------------------------e
